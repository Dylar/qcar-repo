    ...
    @MockkBean(relaxed = true) private val service: VideoInfoService
        ...
        inner class GETVideoInfo {
            ...
            fun `get no video info`() {
                ...
                every { service.getVideoInfo(any(), any(), any()) }
                    .answers {
                ...
                mockMvc.get("$VIDEO_INFO_URL_V1/${info.brand}/${info.model}/${info.name}")
                    .andDo { print() }
                    .andExpect { status { isNotFound() } }

                verify(exactly = 1) { service.getVideoInfo(info.brand, info.model, info.name) }
            }
            ...
            fun `get video info`() {
                ...
                every { service.getVideoInfo(info.brand, info.model, info.name) }
                    .answers { info }
